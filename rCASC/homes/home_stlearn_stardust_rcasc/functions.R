...
clustering=function(matrixName,tissuePositionFile,morphologicalFeaturesFile,
    spotDistanceTransformationWeight,nPerm,permAtTime,percent,nCluster,logTen,
    format,separator,pcaDimensions){
    ...
    morphologicalFeatures <- as.matrix(read.table(paste("./../",morphologicalFeaturesFile,sep=""),header=TRUE,sep="\t",row.names=1))
    morphologicalFeatures <- morphologicalFeatures[,sort(colnames(morphologicalFeatures))]
    ...
    distCoord <- dist(tissuePosition,method="minkowski",p=2)
    distCoord <- distCoord*((max(distPCA)*as.double(spotDistanceTransformationWeight))/(max(distCoord)))

    distMorph <- dist(morphologicalFeatures,method="minkowski",p=2)
    distMorph <- distMorph*(max(distPCA)/max(distMorph))

    finalDistance <- as.matrix(distPCA + distCoord + distMorph)
    ...
    pbmc.new <- RunTSNE(object = distPCA + distCoord + distMorph)
    ...
    for(i in 1:cycles){
        ...
        do
        nohup Rscript ./../../../home/permutation.R ",percent," ",matrixName," ",tissuePositionFile," ",morphologicalFeaturesFile," ",spotDistanceTransformationWeight," ",format," ",separator," ",logTen," ",pcaDimensions," ",sparse," $(($X +",(i-1)*permAtTime," )) & 
